# Chiron GoRound TC: Source Files

Source files of Chiron GoRound TC.
...
```bash
docker build -t chiron-goround-builder:latest .
```
...
```bash 
docker run --rm -it -v $(pwd):/source -v /path/to/build/output:/build chiron-goround-builder
```
...
```bash 
docker run --rm -it -v $(pwd):/source --entrypoint /source/bin/build_var_gf.sh chiron-goround-builder
```
