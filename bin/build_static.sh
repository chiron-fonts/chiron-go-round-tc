...
build_static_otf() {
  ...
  mkdir -p "$BUILD_STATIC_TTF_DIR"
    ...
    do
      ...
      FILENAME="${PREFIX}-${INSTANCE_FILENAMES[$WEIGHT]}${FILENAME_SUFFIX}"
      ...
      echo "[STATIC OTF] (W=$WEIGHT) Building OTF: $CMD"
      $CMD

      CMD="otf2ttf /tmp/${FILENAME}.otf /tmp/${FILENAME}.ttf"
      echo "[STATIC OTF] (W=$WEIGHT) Converting OTF to TTF: $CMD"
      $CMD
...
build_static_otf "regular" "ChironGoRoundTC"
